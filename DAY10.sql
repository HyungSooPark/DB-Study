-- PL/SQL의 FRO 반복문을 이용하여 EMPLOYEE 테이블에서
-- 200번부터 210번까지의 사원의 아이디, 사원명, 이메일 출력
DECLARE
	E EMPLOYEE%ROWTYPE;
BEGIN
...
-- [사용형식]
-- CREATE [OR REPLACE] PROCEDURE 프로시저명 (매개변수 1 [IN/OUT/IN OUT] 자료형[, 매개변수 2 자료형])
--							IN : 프로시저에서 사용할 변수 값을 외부에서 받아올 때 사용하는 모드
--							OUT : 프로시저를 실행한 결과를 외부로 추출할 때 사용하는 모드
--							IN OUT : 두 가지 기능을 선택해서 사용할 수 있는 모드
-- IS	-> DECLARE(선언부)
--	변수선언;
-- BEGIN
--	실행할 코드;
...
-- 제품 등록 --
-- INSERT INT PRODUCT VALUES(SEQ_PRODUCT.NEXTVAL, '제품명', '브랜드', '가격', DEFAULT);
INSERT INTO PRODUCT
