...
-- ORA-02292: integrity constraint (TEST.SYS_C007125) violated - child record found
DELETE FROM DEPARTMENT WHERE DEPT_ID = 'D1';

-- 1) 제약조건 관련 컬럼에 삭제 옵션 적용하기
...
DISABLE CONSTRAINT SYS_C007103;

-- 제약조건 활성화
ALTER TABLE EMPLOYEE
ENABLE CONSTRAINT SYS_C007103;
...
-- COMMIT : 트랜잭션이 종료될 때 정상적으로 종료 되었다면 변경사항을 DB에 저장하겠다.
-- ROLLBACK : 트랜잭션이 작업 중 오류가 발생할 경우 잘못 작업한 내역을 취소하겠다.
-- SAVEPOINT 임시저장소명 : 현재 트랜잭션 수행 중 특정 구역을 나누어 현재까지 진행된 부분만 별도로 중간에 저장하겠다.
-- ROLLBACK TO 임시저장소명 : 트랜잭션 수행 중 임시 저장된 SAVEPOINT를 찾아 해당 부분까지 원상복구할 때 사용
COMMIT;
...
-- DROP --
-- 객체의 특정요소를 제거하거나 객체 자체를 제거할 때 사용하는 명령어
-- [형식 1]
-- ALTER 객체 객체명
-- DROP 지우고자 하는 요소
...
INSERT INT V_EMP_SAL
VALUES(901,'홍길동',3000000,40000000);
...
-- VIEW 생성 시 설정 옵션
-- OR REPLACE : 기존에 동일한 이름의 뷰가 있을 경우 해당 뷰를 덮어씌우고, 없으면 새로 만든다.
-- FORCE / NO FOREC : 서브쿼리에 사용되는 테이블이 존재하지 않아도 뷰를 강제 생성할 것인지 결정
...
-- SEQUENCE : 시퀀스
-- 1,2,3,4,5 ... 형식으로 숫자 데이터를 자동으로 카운트하는 객체
 ...
 * [CACHE 바이트 크기 | NOCACHE] : 값을 미리 구하여 놓고 다음 값을 반영할 때 활용하는 설정
 */
...
VALUES(SEQ_EID.NEXTVAL, '박형수','121212-1234567','PARK@KH.OR.KR','01012344566',
...
INSERT INTO EMPLOYEE
VALUES(SEQ_EID.NEXTVAL, '홍길동','111111-1234567','HONG@KH.OR.KR','01012344566',
		'D9','J7','S1',5000000,0.1,200,SYSDATE,NULL,DEFAULT);
INSERT INTO EMPLOYEE
VALUES(SEQ_EID.NEXTVAL, '임꺽정','570120-1234567','LIM@KH.OR.KR','01012344566',
		'D9','J7','S1',5000000,0.1,200,SYSDATE,NULL,DEFAULT);
INSERT INTO EMPLOYEE
VALUES(SEQ_EID.NEXTVAL, '유관순','990519-1234567','YOO@KH.OR.KR','01012344566',
		'D9','J7','S1',5000000,0.1,200,SYSDATE,NULL,DEFAULT);
INSERT INTO EMPLOYEE
VALUES(SEQ_EID.NEXTVAL, '김유신','890917-1234567','KIM@KH.OR.KR','01012344566',
		'D9','J7','S1',5000000,0.1,200,SYSDATE,NULL,DEFAULT);

-- CYCLE / CACHE
-- CYCLE : 시퀀스의 값이 최소값 혹은 최대값에 도달했을 때 다시 반대의 값부터 시작하는 옵션
...
SELECT SEQ_CYCLE.NEXTVAL FROM DUAL; -- 최대값 230 도달 후 다시 최소값 15부터 시작
