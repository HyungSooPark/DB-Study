...
SELECT  EMP_ID "사 번", EMP_NAME "(사원명)" FROM EMPLOYEE;

-- 실습문제 1
-- EMPLOYEE 테이블에서 사원 번호가 205번인 사원의
-- 사원명, 이메일, 급여, 입사일자를 조회하시오 (단, 조회하는 컬럼명에 별칭 부여)
SELECT EMP_NAME 이름, EMAIL 이메일, SALARY 급여, HIRE_DATE 입사일
FROM EMPLOYEE WHERE EMP_ID='205';

-- 컬럼값을 사용하여 계산식 적용
-- 컬럼값이 만약 NULL이라면 어떤 연산을 처리하여도 결과는 NULL
...
-- 급여를 200만웝도마 많이 받는 직원의 사번, 사원명, 급여를 조회

-- 1) '해외영업2부' 부서의 코드 찾기
SELECT * FROM DEPARTMENT;
SELECT DEPT_ID FROM DEPARTMENT WHERE DEPT_TITLE='해외영업2부'; --D6

-- 2) 조회한 부서코드를 사용하여 사원들 중 급여를 200만원보다 많이 받는 직원 조회
SELECT EMP_ID 사번, EMP_NAME 사원명, SALARY 급여
FROM EMPLOYEE WHERE DEPT_CODE='D6' AND SALARY>2000000;
...
SELECT EMP_ID || '을 가진 사원의 이름은 ' || EMP_NAME || '입니다.' "사번/사원명" FROM EMPLOYEE;
...
-- 직원의 사번, 사원명, 부서코드, 직급코드, 급여정보를 조회
SELECT EMP_ID 사번, EMP_NAME 사원명, DEPT_CODE 부서코드, JOB_CODE 직급코드, SALARY 급여정보 FROM EMPLOYEE
...
-- EMPLOYEE에서 주민등록번호 기준 남성인 사원의 정보만 조회
...
-- EMPLOYEE에서 '이'씨 성을 가진 직원 모두 조회하기
...
-- 직급이 'J7'인 사원의 사번, 사원명, 직급코드, 급여정보 조회
...
-- DUAL : 임시 테이블(가상의 테이블)
-- SELECT 문에서 리터럴을 활용한 계산식을 테스트하고자 할 때 사용하는 임시 테이블

SELECT LENGTH('박형수'), LENGTHB('박형수'),
	   LENGTH ('HELLO'), LENGTHB('HELLO')
FROM DUAL;

-- INSTR : 주어진 값에서 원하는 문자가 몇번쨰인지 찾아 반환하는 함수
...
-- LTRIM / RTRIM
-- 현재 부여된 컬럼 값이나 특정 값으로부터 특정 문자를 찾아 지워주는 함수
...
-- REPLACE : 주어진 값에서 특정 문자를 변경할때 사용하는 함수
...
SELECT EMP_NAME "이름",
	   ...
	   CONCAT(LTRIM(SUBSTR(EMP_NO,5,2),'0'),'일') "생일"
FROM EMPLOYEE;
