...
COMMENT ON COLUMN MEMBER.M_ID IS '회원비밀번호';

INSERT INTO MEMBER
VALUES(1,'USER01','PASS01','홍길동','M','010-1234-5678',TO_DATE('94/03/26','RR/MM/DD'));

INSERT INTO MEMBER
VALUES(2,'USER02','PASS02','임꺽정','M','010-2222-2222',TO_DATE('97/10/22','RR/MM/DD'));
...
-- 컬럼 레벨일 경우
-- 컬럼명 자료형(길이) [CONSTRAINT 제약조건명] REFRENCES 참조할 테이블명[(참조할 컬럼명)] [삭제옵션]
-- 테이블 레벨일 경우
-- [CONSTRAINT 제약조건명] FOREIGN KEY(적용할 컬럼명) REFRENCES 참조할 테이블명[(참조할 컬럼명)] [삭제옵션]
...
INSERT INTO USER_FOREIGN_KEY VALUES(6,'QWE','QWE1','PARK','M',10);
...
-- 연습 1. USER_FOREGIN_KEY + ON DELETE SET NULL
CREATE TABLE USER_FOREIGN_KEY(
	USER_NO NUMBER PRIMARY KEY,
	USER_ID VARCHAR2(20),
	USER_PWD VARCHAR2(20),
...
--	JOB_CODE에 외래키 제약조건 추가
--	참조할 컬럼은 JOB.JOB_CODE
--	SAL_LEVEL에 외래키 제약조건 추가
...
-- 참조할 컬럼명 명시하지 않을 경우 해당 테이블의 PRIMARY KEY 참조
...
VALUES(500,'한예인','991213-2234556','han@kh.or.kr','01011112222','D1','J7',
	   'S4',3100000,0.1,'200',SYSDATE,NULL,DEFAULT);
SELECT * FROM EMPLOYEE WHERE EMP_NAME='한예인';

-- 컬럼을 생략하고 사용하기(모든 컬럼에 값 추가)
INSERT INTO EMPLOYEE 
VALUES(900,'박형수','940309-1234223','park@kh.or.kr','01022334444','D1','J7',
...
-- EMPLOYEE 테이블에서 D1 부서에서 근무하는 직원들의
...
-- UPDATE + SUBQUERY
-- 여러 행을 변경하거나, 여러 컬럼의 값을 변경하고자 할 때 서브쿼리를 사용하여 UPDATE를 사용할 수 있다.
...
-- 노옹철, 전형돈, 정중하, 하동운의 급여와 보너스를 유재식 사원과 같은 급여, 보너스로 수정
-- 단, 다중열 서브쿼리로 구현
UPDATE EMP_SALARY
...
-- ORA-02291: integrity constraint (TEST.SYS_C007125) violated - parent key not found
-- 외래키 제약조건 위배
UPDATE EMPLOYEE
SET DEPT_CODE='D0'
WHERE DEPT_CODE='D6';
